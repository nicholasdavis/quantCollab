...
function displayLineLengthChart(){
  ...
  for(var i = 0; i<analyzer.turns.length; i++){
    if(analyzer.turns[i].activePlayer ==player1){
      p1LineLength.push({"y": analyzer.turns[i].currentLine.length})
    }else{
      p2LineLength.push({"y":analyzer.turns[i].currentLine.length})
    }
    if(analyzer.turns[i].currentLine.length > maxY){
      maxY = analyzer.turns[i].currentLine.length; 
    }
    if(analyzer.turns[i].currentLine.length < minY){
      minY = analyzer.turns[i].currentLine.length; 
    }
  }
...
$('#canvas').mouseup(function(e){
  ...
  //line = new Array(); 
    ...
    turn = new Turn(activePlayer, inactivePlayer, lines[lineCounter], lines[lineCounter-1], turnStart, turnEnd)
